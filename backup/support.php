...
$tickets = fetchAll("SELECT * FROM support_tickets ORDER BY created_at DESC", [], '');
?><!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8">
  <title>Flexnet – Support Tickets</title>
...
<div class="admin-shell">
  <aside class="admin-sidebar" id="adminSidebar">
    ...
    <ul class="admin-nav">
      ...
      <li class="admin-nav-item"><a href="support.php" class="admin-nav-link active"><i class="bi bi-life-preserver"></i><span>Support</span></a></li>
  ...
  <div class="admin-main">
    <header class="admin-topbar">
      <div class="topbar-left">
        ...
        <div><div class="topbar-title">Support Tickets</div><div class="topbar-subtitle">Manage customer support requests.</div></div>
      </div>
      <div class="topbar-right">
        <div class="topbar-search"><input type="text" id="searchTickets" placeholder="Search tickets…"><i class="bi bi-search"></i></div>
    ...
    <main class="admin-content">
      <div class="page-header">
        <div><div class="page-title">Support Tickets</div><div class="page-subtitle">View and manage customer support requests.</div></div>
        ...
        <div class="card-header">
          <div style="display:flex;justify-content:space-between;align-items:center;">
            <div>
              ...
              <div class="page-subtitle">Customer, subject, status and dates.</div>
            </div>
            <div class="page-subtitle">Showing <span id="ticketCount"><?php echo count($tickets); ?></span> ticket(s)</div>
          </div>
        </div>
                ...
                <tr data-ticket-id="<?php echo $ticket['id']; ?>">
                  ...
                  <td>
                    <span class="badge <?php
                      $status_classes = ['open' => 'badge-info', 'in_progress' => 'badge-warn', 'closed' => 'badge-success', 'resolved' => 'badge-success'];
                      echo $status_classes[$ticket['status']] ?? 'badge-info';
                    ?>"><?php echo ucfirst(str_replace('_', ' ', $ticket['status'])); ?></span>
                  </td>
                  <td><?php echo date('M d, Y', strtotime($ticket['created_at'])); ?></td>
                  <td><?php echo date('M d, Y', strtotime($ticket['updated_at'])); ?></td>
                  <td>
                    <button class="btn-icon view-ticket" data-id="<?php echo $ticket['id']; ?>" data-customer="<?php echo htmlspecialchars($ticket['customer_name']); ?>" data-subject="<?php echo htmlspecialchars($ticket['subject']); ?>" data-status="<?php echo $ticket['status']; ?>" title="View"><i class="bi bi-eye"></i></button>
                    <button class="btn-icon edit-ticket" data-id="<?php echo $ticket['id']; ?>" data-status="<?php echo $ticket['status']; ?>" title="Update Status"><i class="bi bi-pencil"></i></button>
                  </td>
                </tr>
              <?php endforeach; ?>
            </tbody>
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <div class="mb-2">
          ...
          <p id="viewCustomer" style="font-size:0.9rem;margin:0;"></p>
        </div>
        <div class="mb-2">
          <label class="form-label">Subject</label>
          <p id="viewSubject" style="font-size:0.9rem;margin:0;"></p>
        </div>
        <div class="mb-0">
...
<div class="modal fade" id="editTicketModal" tabindex="-1">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      <div class="modal-header">
        <h6 class="modal-title">Update Ticket Status</h6>
        <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
      </div>
...
<div class="modal fade" id="filterModal" tabindex="-1">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      <div class="modal-header">
        <h6 class="modal-title">Filter Tickets</h6>
        <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
      </div>
      <div class="modal-body">
        <div class="mb-3">
...
document.querySelectorAll('.view-ticket').forEach(btn => {
  btn.addEventListener('click', () => {
    document.getElementById('viewCustomer').textContent = btn.dataset.customer;
    document.getElementById('viewSubject').textContent = btn.dataset.subject;
    document.getElementById('viewStatus').innerHTML = `<span class="badge badge-info">${btn.dataset.status}</span>`;
...
document.querySelectorAll('.edit-ticket').forEach(btn => {
  btn.addEventListener('click', () => {
    ...
    document.getElementById('editTicketForm').querySelector('[name="status"]').value = btn.dataset.status;
...
document.getElementById('editTicketForm').addEventListener('submit', async (e) => {
  ...
  fd.append('action', 'update_ticket_status');
  try {
...
// Search Tickets
document.getElementById('searchTickets').addEventListener('input', async (e) => {
  ...
  try {
    const res = await fetch(`includes/api-handlers.php?action=search_tickets&q=${encodeURIComponent(q)}`);
    const json = await res.json();
    if (json.success) {
      const tbody = document.getElementById('ticketsTableBody');
      tbody.innerHTML = '';
      json.data.forEach(ticket => {
        const statusClasses = {'open': 'badge-info', 'in_progress': 'badge-warn', 'closed': 'badge-success', 'resolved': 'badge-success'};
        const row = `<tr data-ticket-id="${ticket.id}">
          <td>${ticket.customer_name}</td>
          <td>${ticket.subject}</td>
          <td><span class="badge ${statusClasses[ticket.status] || 'badge-info'}">${ticket.status.replace('_', ' ')}</span></td>
          <td>${new Date(ticket.created_at).toLocaleDateString()}</td>
          <td>${new Date(ticket.updated_at).toLocaleDateString()}</td>
          <td>
            <button class="btn-icon view-ticket" data-id="${ticket.id}" data-customer="${ticket.customer_name}" data-subject="${ticket.subject}" data-status="${ticket.status}"><i class="bi bi-eye"></i></button>
            <button class="btn-icon edit-ticket" data-id="${ticket.id}" data-status="${ticket.status}"><i class="bi bi-pencil"></i></button>
          </td>
        </tr>`;
        tbody.innerHTML += row;
      });
      document.getElementById('ticketCount').textContent = json.data.length;
...
document.getElementById('applyFilterBtn').addEventListener('click', async () => {
  ...
  try {
    const res = await fetch(`includes/api-handlers.php?action=filter_tickets&status=${status}`);
    const json = await res.json();
    if (json.success) {
      const tbody = document.getElementById('ticketsTableBody');
      tbody.innerHTML = '';
